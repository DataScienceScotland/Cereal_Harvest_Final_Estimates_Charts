...
# for y axis labels- commented out as want the current year to display
...
area
area <- area + geom_line(aes(x = Year, y = Area), data = area_cereals, lwd = 2.5)
area
...
prod <-prod + 
  ...
  scale_x_continuous(limits = c((CurrentYear-9), CurrentYear), breaks =c((CurrentYear-9):CurrentYear), 
                     labels = xaxislabels)+
  scale_color_manual(values=c("#28a197","#0065bd", "#5eb135", "#0e450b"  ))
prod <- prod + geom_line(aes(x = Year, y = Production), data = prod_cereals, lwd = 2.5)
prod
...
yield <-yield +
  ...
  scale_color_manual(values=c("#28a197","#0065bd", "#5eb135", "#0e450b"  ))
yield <- yield +geom_line(aes(x = Year, y = Yield), data = yield_cereals, lwd = 2.5) 
